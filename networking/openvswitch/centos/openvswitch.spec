...
# wordaround(LLU) - 'make check' failed in mockchain environment, but succeeded in mock,
...
%if %{with dpdk}
Patch1: ovs-vswitchd.system.service.patch
%endif
...
%else
BuildRequires: libpcap-devel numactl-devel dpdk-devel
%endif
%endif
%endif
...
%if %{with dpdk}
%ifarch %{dpdkarches}
# install dpdk usertools
  install -d -m 0755 $RPM_BUILD_ROOT%{_datadir}/openvswitch/dpdk-usertools
%if %{with dpdkstatic}
  install -p -m 0755 $(pwd)/%{dpdkdir}-%{dpdkver}/usertools/* $RPM_BUILD_ROOT%{_datadir}/openvswitch/dpdk-usertools
%else
  install -p -m 0755 %{_datadir}/dpdk/usertools/* $RPM_BUILD_ROOT%{_datadir}/openvswitch/dpdk-usertools
%endif
%endif
%endif
...
%if %{with dpdk}
%ifarch %{dpdkarches}
%{_datadir}/openvswitch/dpdk-usertools
%endif
%endif
...
%changelog
* Tue Jan 30 2018 Lianhao Lu <lianhao.lu@intel.com> - 2.8.1-3.tis.1
- Adjusted for TiC
- Include dpdk usertools in the final package
